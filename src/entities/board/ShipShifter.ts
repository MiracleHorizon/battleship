...
export class ShipShifter {
  ...
  /**
   * @param cellMatrix - matrix of cells on the board.
   ...
   */
  constructor({ cellMatrix, fleet, ship, cell, prev, next }: Constructor) {
  ...
  private shiftHorizontalShip(): boolean {
    ...
    for (let cellRow = 0; cellRow < this.cellMatrix.length; cellRow++) {
      if (cellRow !== this.cell.row) {
        continue
      }

      const row = this.cellMatrix[cellRow]
